  ...
  constructor(props) {
    ...
    render() {
      ...
      const getPasswordError = value => {
        ...
        else if (!value.match(/\d/)) return 'Password must contain at least one number'
        else if (!value.match(/^(?=.*[a-z])(?=.*[A-Z])/)) return 'Password must contain both uppercase and lowercase letters'
        else return null
      }
      ...
      <Section id="form">
        <Title small>
          Pick a username and a password of at least 8 characters which includes both upper and lower case, and at least one number
        </Title>
        ...
        <div>
          ...
          {usernameError ? <div style={{color: 'pink'}}>{usernameError}</div> : null}
          </div>
          <div>
          <label for="password" style={{color: 'white'}}>
            Password:
            <Input long type="password" ref={input => {passwordInput = input}} onChange={handlePasswordChange} />
          </label>
          {passwordError ? <div style={{color: 'pink'}}>{passwordError}</div> : null}
          </div>
          <div>
          <label for="password" style={{color: 'white'}}>
            Confirm password:
          ...
          {passwordCheckError ? <div style={{color: 'pink'}}>{passwordCheckError}</div> : null}
          </div>
          <Button form disabled={Boolean(usernameError || passwordError || passwordCheckError)} type="submit">Submit</Button>
        </form>
      </Section>
    );
  }
